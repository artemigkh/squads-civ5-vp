...
function UpdateMouse(gridX, gridY)
    print("in squads update mouse function");
    if gridX == nil then
    ...
    if currentMode == SQUADS_MODE_UNIT_MANAGEMENT then
        print("updating mouse for unit management mode")
        local iPlayer = Game.GetActivePlayer();
        local pPlayer = Players[ iPlayer ];
        ...
        if plot ~= nil then
            ...
            print("is box selecting?", bIsBoxSelecting)
...
function HandleKeyDown(wParam)
    print("received key press ", wParam)

    if wParam == Keys.VK_ESCAPE then
        print("Received escape, leaving current view")
        SetSquadsMode(SQUADS_MODE_NONE);
    end
...
-- LuaEvents.SQUADS_MANAGE_UNITS_MODE_KEY_DOWN.Add(HandleKeyDown);
-- LuaEvents.SQUADS_MANAGE_BASES_MODE_KEY_DOWN.Add(HandleKeyDown);
-- LuaEvents.SQUADS_MOVE_UNITS_MODE_KEY_DOWN.Add(HandleKeyDown);



-- ========== Manage Mode Handlers ==========
function HandleLeftClickUp(wParam, lParam)
    print("handing left click squads");
...
function HandleNumericHotkey(number, bCtrl)
    ...
    print("last pushed at ", lastPressTime);
...
function HandleFocusSquad(number)
    print("Double press has occurred", number);
    local iPlayer = Game.GetActivePlayer();
    local pPlayer = Players[ iPlayer ];
    currentSquadNumber = number;
...
function HandleCancelMoveSquad(wParam, lParam)
    print("handling cancelling moving squad")
    SetSquadsMode(SQUADS_MODE_NONE);
end
...
function HighlightAllSquadStatus(pPlayer)
    ...
    for unit in pPlayer:Units() do
        -- print(string.format("handling unit %s with squad num %i", unit:GetName(), unit:GetSquadNumber()))
        if unit:GetSquadNumber() == -1 then 
            -- do nothing
        elseif unit:GetSquadNumber() == currentSquadNumber then
            -- White if already in the current squad
            -- print("white from being in same squad")
            Events.SerialEventHexHighlight(ToHexFromGrid(Vector2(unit:GetX(), unit:GetY())), true, Vector4(1, 1, 1, 1));
        else
            -- Orange if already in a squad
            -- print("orange from being in a squad")
...
function SquadEntryClicked(squadNumber)
    print("Squad entry clicked, updating active squad to ", squadNumber);
...
function SquadRenamed(newName)
    ...
    print("Current Squad renamed to ", newName);
...
function HandlePressChangeHomeBaseButton ()
  print("handle change home base button clicked");
...
function HandlePressViewHomeBaseButton()
  ...
  print("handle view home base button clicked");
...
function BuildUnitList(iSelectedUnit)
    ...
    print("Current squad number is ", currentSquadNumber);
    print("Rebuilding unit list with squad number", currentSquadNumber);
    for unit in getSquadUnits(pPlayer, currentSquadNumber)
    do
        print("Adding unit ", unit:GetName())
        local instance;
...
function OnAdditionalInformationDropdownGatherEntries(additionalEntries)
  table.insert(additionalEntries, {text=Locale.ConvertTextKey("TXT_KEY_SQUADS_TITLE"), call=OnDiploCornerPopup, art="MilitaryIcon.dds"})
end
